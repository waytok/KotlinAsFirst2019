...
fun sibilants(inputName: String, outputName: String) {
    val outputStream = File(outputName).bufferedWriter()
    val hissing = "ЖЧШЩжчшщ"
    var previousSymbol = '0'
    for (symbol in File(inputName).readText()) {
        if (hissing.contains(previousSymbol)) when (symbol) {
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (line in File(inputName).readLines()) {
        repeat((maxLineLength - line.trim().length) / 2) { outputStream.write(" ") }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    var words = ""
    var maxLength = 0
    var repetitions = false
    for (word in File(inputName).readText().split(Regex("""\s+"""))) {
        for (symbol in word) if (word.count { it.toUpperCase() == symbol.toUpperCase() } != 1) repetitions = true
        if (word.length == maxLength && !repetitions) {
            words += ", $word"
        } else if (word.length > maxLength && !repetitions) {
